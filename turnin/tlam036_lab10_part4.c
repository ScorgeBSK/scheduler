...
enum Custom_States { waitInit, waitButtonRelease, waitComboInput, enterComboRelease, waitComboRelease };
...
int customTick(int state){
	...
	switch(state){
		case waitInit:
			state = (button && output == 0x0E) ? waitRelease : waitInit;	
			break;
		case waitRelease:
			if(button && output == 0x0E){
				state = waitRelease;
			}
			else {
				state = waitComboInput;
			}
			break;
		case waitComboInput:
			if(j < 4) {
		...
		case enterComboRelease:
			if(j < 4) {state = waitComboInput;} else {state = waitInit;}
		       break;	       
		default:
			state = waitInit;
			break;
...
int main(void) {
	...
	while (1) {
		...
		PORTB = door;
		PORTD = led | led1 | j << 3;	//debugging indicators
		while(!TimerFlag){
			TimerFlag = 0;
		}
	}
